...
export class PeopleProvider {
  ...
  async updatePerson (
    ...
    if (personData?.ci) {
      ...
      if (personFound) { throw new HttpException('Person Found', HttpStatus.NOT_ACCEPTABLE) }
    }
    const personFound = await this.peopleService.update(personId, personData)
    if (personFound.affected == 0) { throw new HttpException('Person not found', HttpStatus.NOT_FOUND) }
    return personFound
  }

  async deletePerson (personId: number) {
    const personFound = await this.peopleService.delete(personId)
    if (personFound.affected == 0) { throw new HttpException('Person not found', HttpStatus.NOT_ACCEPTABLE) }
    return personFound
  }
}
